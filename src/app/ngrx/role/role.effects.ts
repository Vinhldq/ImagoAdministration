...
export class RoleEffect {
    ...
    this.actions.pipe(
      ...
      switchMap((action) => {
        return this.roleService.getAllRole(action.token, action.page).pipe(
          map((roles: any) =>
            RoleActions.getAllRoleSuccess({ roleList: roles })
          ),
          catchError((error) => {
  ...
  $getAllSearchRole = createEffect(() =>
    this.actions.pipe(
      ofType(RoleActions.getAllSearchRole),
      switchMap((action) => {
        return this.roleService
          .getAllSearchRole(action.token, action.page)
          .pipe(
            map((roles: any) => RoleActions.getAllSearchRoleSuccess({ roles })),
            catchError((error) => [
              RoleActions.getAllSearchRoleFailure({ error }),
            ])
          );
      })
    )
